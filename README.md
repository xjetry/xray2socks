...
`add` 的端口和 bind 都可省略：端口从 1081 起跳过配置里已用的和系统占用的；bind 默认 `0.0.0.0`，多个地址用逗号分隔、不区分 v4/v6，IPv6 用方括号。`edit --bind` 可改监听地址。`add` / `edit` / `remove` 只写配置，不影响运行中的 Xray；运行中会提示用 `x2socks serve` 应用新配置。

启动与停止（编辑与运行状态独立控制）：

```bash
x2socks serve    # 后台启动/重启 Xray（按最新配置，命令立即返回，Xray 脱离终端存活）
x2socks stop     # 停止后台 Xray（幂等）
```

`list` 的 ID 从 1 开始，并显示 generate204 延迟。`test {uri}` 只检查连通性，不写配置。URI 合法即可保存，不通也不拦。`edit` 的 `--uri`、`--port`、`--bind` 至少填一个。

网页管理（可选常驻，只提供页面；启动时顺带确保 Xray 在跑，退出页面不影响后台 Xray）：

```bash
x2socks web --bind 127.0.0.1 --web-addr 127.0.0.1:8080
```
